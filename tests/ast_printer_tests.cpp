...
TEST_CASE("Ast Printer Test", "[AST Printer]") {
    ...
    SECTION("Literals") {
        LiteralExpression l(TokenType::NUMBER, "69");
        CHECK(printer.print(l) == "69");
    }

    SECTION("Tree Structure Verification") {
        std::unique_ptr<Expression> l1 = std::make_unique<LiteralExpression>(TokenType::NUMBER, "69");
        std::unique_ptr<Expression> l2 = std::make_unique<LiteralExpression>(TokenType::NUMBER, "55");
        std::unique_ptr<Expression> l3 = std::make_unique<LiteralExpression>(TokenType::NUMBER, "2");
    ...
    SECTION("Tree Structure Verification 2") {
        std::unique_ptr<Expression> l1 = std::make_unique<LiteralExpression>(TokenType::TRUE, "true");
